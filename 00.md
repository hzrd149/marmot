...
- **`relays`**: Relay URLs where this KeyPackage is published. MUST contain at least one valid WebSocket URL (ws:// or wss://). Used for KeyPackage discovery.
...
- **Last resort KeyPackages**: The private init_key MUST be retained as long as the last resort KeyPackage remains published or you have reason to believe it will be needed to decrypt Welcome messages. The init_key MUST be deleted only when the last_resort KeyPackage is rotated (i.e., a new `kind:30443` event is published under the same `d` identifier).
...
Clients MUST be able to read and process `kind:443` KeyPackage events from relays during the transition period. When querying for a user's KeyPackages, clients SHOULD query for both kinds simultaneously:
...
When both `kind:443` and `kind:30443` events are present for the same user, clients MUST prefer `kind:30443` events. Within `kind:443` events, clients MUST prefer the event with the highest `created_at`.

#### Publishing

Clients MUST only publish `kind:30443` events. Publishing new `kind:443` events is not permitted.

> **Note**: Because `kind:443` events have no `d` tag, they cannot be replaced — only deleted via NIP-09. Clients that previously published `kind:443` events SHOULD send NIP-09 deletion events for those KeyPackages once they have published equivalent `kind:30443` replacements and verified relay propagation.
