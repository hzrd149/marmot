...
`kind:30443` is an addressable event. For each combination of `pubkey`, `kind`, and `d` tag value, relays store only the most recent version. This means publishing a new `kind:30443` event under the same `d` tag value automatically supersedes the old one on all relays — no explicit deletion is required for rotation.
...
Rotation does NOT require sending a NIP-09 deletion event. The new event replaces the old one on relays automatically.
