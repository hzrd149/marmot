...
4. If multiple candidates still tie for highest `created_at`, choose the one with the lexicographically smallest `id`
...
`kind:30443` is the canonical KeyPackage format defined by this specification. During a migration window where legacy `kind:443` KeyPackages must still interoperate with `kind:30443`, clients SHOULD query for both kinds simultaneously:
...
When both `kind:443` and `kind:30443` events are present for the same user, clients MUST prefer valid `kind:30443` events. If no valid `kind:30443` is available, clients MAY fall back to the freshest valid `kind:443` event. Within `kind:443` events, clients MUST prefer the event with the highest `created_at`. Deployment-specific cutover dates and rollout plans SHOULD be documented outside this MIP; the current Marmot migration plan is documented in [docs/migrations/2026-05-01-kind-30443-cutover.md](docs/migrations/2026-05-01-kind-30443-cutover.md).

#### Publishing

Clients that need interoperability with legacy peers MUST dual-publish equivalent KeyPackages as both `kind:30443` and `kind:443` during the migration window. The legacy `kind:443` event MUST carry the same KeyPackage content and the same applicable tags as the `kind:30443` event, except for the `d` tag, which `kind:443` does not support.

Once a deployment's migration window has ended, clients MUST publish only `kind:30443` KeyPackage events. Publishing new `kind:443` KeyPackages is not permitted after the deployment-specific cutover.
