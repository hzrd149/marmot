...
**Required fields:**
- **`content`**: TLS-serialized `KeyPackageBundle` from your MLS implementation, encoded as either hex or base64. The encoding format is specified by the `encoding` tag.
- **`encoding`**: Specifies the encoding format of the `content` field:
  ...
  The tag-based approach eliminates ambiguity for strings like `deadbeef` that are valid in both hex and base64 formats but decode to completely different bytes. New implementations SHOULD use base64 encoding.
