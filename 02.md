...
- **`e` tag**: ID of the KeyPackage Event used to add this user. This is ordinarily a `kind:30443` event. During any active migration window (see [MIP-00](00.md) and the migration guide), it MAY be the ID of a legacy `kind:443` KeyPackage event when no valid `kind:30443` was available at invitation time.
...
5. **Rotate KeyPackage**: Publish a new `kind:30443` event to replace the consumed KeyPackage on relays (see [MIP-00](00.md) KeyPackage Lifecycle Management). If the consumed KeyPackage was a `kind:30443` event, publish the replacement under the **same `d` tag identifier**. If the consumed KeyPackage was a legacy `kind:443` event (no `d` tag), publish a fresh `kind:30443` event with a new random `d` identifier.
...
- KeyPackage rotation after joining (publish new `kind:30443` under the same `d` tag when the consumed KeyPackage was `kind:30443`; use a fresh random `d` when the consumed KeyPackage was legacy `kind:443`)
