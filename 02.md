...
- **`e` tag**: ID of the specific `kind:30443` KeyPackage Event used to add this user
...
5. **Rotate KeyPackage**: Publish a new `kind:30443` event under the same `d` tag identifier to replace the consumed KeyPackage on relays (see [MIP-00](00.md) KeyPackage Lifecycle Management)
...
- KeyPackage rotation after joining (publish new `kind:30443` under same `d` tag)
